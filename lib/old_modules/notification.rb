module Notification
  module CardMethods
    ...
    def watching_type?; watcher_pairs(false, :type).member? Account.authorized_id end
    def watching?;      watcher_pairs(false).       member? Account.authorized_id end
    ...
    def watcher_pairs pairs=true, kind=:name, hash={}
      #warn "wp #{inspect} P:#{pairs}, k:#{kind}, uid:#{Account.authorized_id} #{hash.inspect}, OI:#{hash.object_id}"
      ...
      if hash.any?
        ...
        if pairs
          hash.each.reject {|i,wname| i == Account.authorized_id }.map {|i,wname| [ i, wname ] }
        else
          hash.keys
        end
