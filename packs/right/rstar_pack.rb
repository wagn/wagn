...
#      )]
    ...
    if is_self
      cells << ['rule-set', rule_card ? card.label(rule_card.name.trunk_name) : ''] 
    end
        ...
        if is_self
            ...
            div(:class=>'edit-rule-section edit-rule-override') do
              ...
              content_tag(:ul) do
                sifter[:override].map do |set_name|
                  content_tag(:li) { link_to_remote card.label(set_name), :update=>id, 
                    :url=>"/card/view/#{card.cardname.to_url_key}?view=edit_rule&new_rule_set=#{CGI.escape(set_name)}"
                  }
                end.join
              end
            end
          ...
          sections << if set_name = params[:new_rule_set]
            div(:class=>'edit-rule-section edit-rule-new') do
              edit_rule_header('Create', "add new rule for #{card.label(set_name)}:") +
          ...
          sections << if current_rule
            div(:class=>'edit-rule-section edit-rule-current') do
              edit_rule_header('Edit', "change current rule for #{card.label(current_rule_set)}:") +
