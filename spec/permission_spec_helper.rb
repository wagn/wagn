module PermissionSpecHelper
  ...
  def assert_hidden_from( user, card, msg='')
    Card.as(user.id) { assert_hidden( card, msg ) }
  end

  def assert_not_hidden_from( user, card, msg='')
    Card.as(user.id) { assert_not_hidden( card, msg ) }
  end

  def assert_locked_from( user, card, msg='')
    Card.as(user.id) { assert_locked( card, msg ) }
  end

  def assert_not_locked_from( user, card, msg='')
    Card.as(user.id) { assert_not_locked( card, msg ) }
  end
...
class Card
  def writeable_by(user)
    Card.as(user.id) do
    #warn "writeable #{Card.as_user_id}, #{user.inspect}"
      ok? :update
    end
  end
