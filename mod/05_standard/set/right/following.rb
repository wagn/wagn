...
format :html do
  ...
  #   if watched_card = Card.fetch( params[:watch] )
  #     wrap args do
  ...
  #     end
  #   else
  #     card.errors.add :watch_error, "Unknown card #{Env.params[:watch]}"
  #   end
  # end
  #
  # def watch_link watched_card, text, toggle, title, extra={}
  #
  #   #path = wagn_path "update/#{Auth.current.url_key}?watch=#{watched_card.url_key}&toggle=#{toggle}&success[view]=watch"
  #   link_to "#{text}",  path(:card=>Auth.current, :action=>:update, :watch=>watched_card.url_key, :toggle=>toggle,
