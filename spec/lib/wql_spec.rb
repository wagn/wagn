  ...
  describe "nested permissions" do
    ...
    it "are generated by default" do
      Wql.new( { :left=>{:name=>"X"}}).sql.should match(
        "^#{Regexp.escape("(select t.* from cards t  where t.trunk_id in (select id from cards tx  where tx.name = ")}E?#{Regexp.escape("'X' and  (tx.reader_type!='User' and tx.reader_id IN (3,2))  and tx.trash='f'    ) and  (t.reader_type!='User' and t.reader_id IN (3,2))  and t.trash='f'  ORDER BY t.updated_at desc  )")}$" )
    end
    
    it "are not generated inside .without_nested_permissions block" do
       Wql.without_nested_permissions do 
         Wql.new( { :left=>{:name=>"X"}}).sql.should match(
           "^#{Regexp.escape("(select t.* from cards t  where t.trunk_id in (select id from cards tx  where tx.name = ")}E?#{Regexp.escape("'X' and tx.trash='f'    ) and  (t.reader_type!='User' and t.reader_id IN (3,2))  and t.trash='f'  ORDER BY t.updated_at desc  )")}$" )
      end
    end
