...
class Card
  class Log
    ...
    class << self
      ...
      def finish_block
        ...
        return if limit = Cardio.config.performance_logger[:limit] and limit > 0 and duration < limit

        log_msg = "#{ indent log[:level] }(%d.2ms) #{ log[:method] }: #{ log[:message] }" % duration
        log_msg += details log if Cardio.config.performance_logger[:details]
        log_msg += subtree log
  ...
  class << self
    def with_logging cardname, method, message, details, &block
      if Cardio.config.performance_logger and
         Cardio.config.performance_logger[:methods] and
         Cardio.config.performance_logger[:methods].include? method
        Card::Log.start_block :cardname=>cardname, :method=>method, :message=>message, :details=>details
        result = block.call
        Card::Log.finish_block
        result
      else
        block.call
      end
