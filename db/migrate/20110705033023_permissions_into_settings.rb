    ...
    Card.find(:all, :conditions=>"(name like '%*right+*content' or name like '%right+*default') and trash is false").each do |card|
      ...
      [:read, :edit, :delete, :comment].each do |task|
        begin
          ...
          if could && could != can
            new_rule = create_rule "#{base_name}+*right", task, Card.fetch(could.first, :skip_module_loading=>true)
      ...
      [:read, :edit, :delete, :comment].each do |task|
        begin
          ...
          if could && could != can
            ...
            new_rule = create_rule "#{card.name}+*self", task, Card.fetch(could.first, :skip_module_loading=>true)
