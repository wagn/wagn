  ...
  describe "nested permissions" do
    ...
    it "are generated by default" do
      Wql.new( { :left=>{:name=>"X"}}).sql.should match(
        "^#{Regexp.escape("(select t.name from cards t  where t.trunk_id in (select id from cards tx  where tx.name = ")}"+
        "E?#{Regexp.escape("'X' and  (tx.reader_rule_id IN (#{rule_id}))  and tx.trash is false    ) and  (t.reader_rule_id IN (#{rule_id}))  and t.trash is false  ORDER BY t.updated_at desc  )")}$" 
      )
    end
    ...
    it "are not generated inside .without_nested_permissions block" do
       Wql.without_nested_permissions do 
         Wql.new( { :left=>{:name=>"X"}}).sql.should match(
           "^#{Regexp.escape("(select t.name from cards t  where t.trunk_id in (select id from cards tx  where tx.name = ")}"+
           "E?#{Regexp.escape("'X' and tx.trash is false    ) and  (t.reader_rule_id IN (#{rule_id}))  and t.trash is false  ORDER BY t.updated_at desc  )")}$" )
      end
    end
