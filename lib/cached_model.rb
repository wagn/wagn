...
class CachedModel < ActiveRecord::Base
  ...
  def self.find_by_sql(*args)
    #warn "CAched Model find_by_sql: #{args.first}"

    return super unless args.first =~  /^SELECT \* FROM #{table_name}\s+WHERE \(#{table_name}\..?#{primary_key}.? = ?(\d+)?\)/
  ...
  # LWH - this one isn't really model-specific.  does it go here?
  def self.cache(key)
    if CachedModel.use_local_cache? then
      #logger.info("---using local cache for key #{key}---")
      obj = CachedModel.cache_local[key]
    ...
    if CachedModel.use_memcache? then
      ...
      unless obj.nil?
        if CachedModel.use_local_cache? then
          CachedModel.cache_local[key] = obj
        end
        return obj
      end
    end
