...
format :html do
  ...
  def watch_link watched_card, text, toggle, title, extra={}
    ...
    case toggle
    when :off then path_hash[:drop_item] = follow_set_card(watched_card).cardname.url_key
    when :on  then path_hash[:add_item]  = follow_set_card(watched_card).cardname.url_key
    end
...
def type_watched?; Auth.current.fetch(:trait=>:following, :new=>{}).include_item? type_card.fetch(:trait=>:type).cardname.url_key end
def self_watched?; Auth.current.fetch(:trait=>:following, :new=>{}).include_item? fetch(:trait=>:self).cardname.url_key end
def set_watched?;  !(Auth.current.fetch(:trait=>:following, :new=>{}).item_names & set_names).empty?
