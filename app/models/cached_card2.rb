...
class CachedCard 
  ...
  cache_accessor :line_content, :view_content, :footer, :missing
  ...
  def get(field)
    cache.read(property_key(field)) or begin
      value = yield  
      cache.write(property_key(field), value)
      value
    end
  end
