...
describe "On Card Changes" do
  ...
  it "sends notifications of edits" do
    Mailer.should_receive(:deliver_change_notice).with( User.find_by_login('sara'), Card["Sara Watching"], "edited", "Sara Watching", nil )
  ...
  it "sends notifications of additions" do
    ...
    Mailer.should_receive(:deliver_change_notice).with( User.find_by_login('sara'), new_card,"added", "Optic", nil  )
    new_card.save!
  end 
  
  it "sends notification of updates" do
    Mailer.should_receive(:deliver_change_notice).with( User.find_by_login('sara'), Card["Sunglasses"], "updated", "Optic", nil)
    Card["Sunglasses"].update_attributes :type => "Basic"
  end
  
  it "does not send notification to author of change" do
    Mailer.should_receive(:deliver_change_notice).with( User.find_by_login('sara'), Card["All Eyes On Me"],"edited", "All Eyes On Me", nil)
