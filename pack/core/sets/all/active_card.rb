...
#Rails.logger.warn "get trait_var #{var_name}: #{r.inspect}"; r
end
...
Rails.logger.warn "tn saving #{trait_name}, #{trait_card.inspect}, C:#{trait_card.content.inspect}"
      trait_card.save
      instance_variable_set card_attr, nil
      instance_variable_set "@#{trait_name}", ''
      
Rails.logger.warn "tn saved #{trait_name}, #{card_attr.inspect}, #{trait_var? card_attr} "
    end
  end
  true
end
