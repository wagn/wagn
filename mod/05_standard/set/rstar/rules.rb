format :html do
      ...
      <tr class="card-slot open-rule #{rule_view.to_s.sub '_', '-'}">
        <td class="rule-cell" colspan="3">
          <div class="rule-setting">
            ...
            #{ build_link setting_name.to_name, "all rules", :class=>'setting-link', :target=>'wagn_setting' }
          </div>
  ...
  view :show_rule, :tags=>:unknown_ok do |args|
    ...
    if !card.new_card?
      ...
      %{
        <div class="rule-set">
          <label>Applies to</label> #{ build_link set.cardname, set.label }:
        </div>
        #{ _render_core args }
      }
    else
                ...
                %{
                  <li>
                    ...
                    <span class="set-label" #{'current-set-label' if is_current }>
                      #{ build_link set_name.to_name, Card.fetch(set_name).label, :target=>'wagn_set' }
