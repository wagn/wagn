...
def permitted? action
  ...
  if !CardRailtie.config.read_only
...
def permit action, verb=nil
  if CardRailtie.config.read_only # not called by ok_to_read
...
def have_recaptcha_keys?
  @@have_recaptcha_keys = defined?(@@have_recaptcha_keys) ? @@have_recaptcha_keys : 
    !!( CardRailtie.config.recaptcha_public_key && CardRailtie.config.recaptcha_private_key )
end
