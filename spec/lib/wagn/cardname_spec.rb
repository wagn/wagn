...
describe Wagn::Cardname do
  ...
  describe "#key" do
    it "should remove spaces" do
      "this    Name".to_cardname.key.should == "this_name"
    end
    ...
    it "should have initial _ for initial cap" do
      "_This Name".to_cardname.key.should == "this_name"
    end

    it "should singularize" do
      "ethans".to_cardname.key.should == "ethan"
    end

    it "should underscore" do
      "ThisThing".to_cardname.key.should == "this_thing"
    end

    it "should handle plus cards" do
      "ThisThing+Ethans".to_cardname.key.should == "this_thing+ethan"
    end

    it "should retain * for star cards" do
      "*right".to_cardname.key.should == "*right"
    end

    it "should not singularize double s's" do
      "grass".to_cardname.key.should == 'grass'
    end

    it "should not singularize letter 'S'" do
      'S'.to_cardname.key.should == 's'
    end

    it "should handle unicode characters" do
      "Mañana".to_cardname.key.should == 'mañana'
    end

    it "should handle weird initial characters" do
      '__you motha @#$'.to_cardname.key.should == 'you_motha'
      '?!_you motha @#$'.to_cardname.key.should == 'you_motha'
    end

    it "should allow numbers" do
      "3way".to_cardname.key.should == '3way'
    end

    it "internal plurals" do
      "cards hooks label foos".to_cardname.key.should == 'card_hook_label_foo'
    end

    it "should handle html entities" do
      # This no longer takes off the s, is singularize broken now?
      "Jean-fran&ccedil;ois Noubel".to_cardname.key.should == 'jean_françoi_noubel'
    end
  end
  ...
  describe "trait_name?" do
    it "returns true for content codename" do
      "bazoinga+*right+*content".to_cardname.trait_name?(:content).should be_true
    end

    it "handles arrays" do
      "bazoinga+*right+*content".to_cardname.trait_name?(:content, :default).should be_true
    end

    it "returns false for non-template" do
      "bazoinga+*right+nontent".to_cardname.trait_name?(:content).should be_false
    end
  end
