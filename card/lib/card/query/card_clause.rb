  ...
  class Query
    class CardClause < Clause
      ...
      def found_by val
        ...
        cards.each do |ca|
          unless ca && [SearchTypeID,SetID].include?(ca.type_id)
            raise BadQuery, %{"found_by" value needs to be valid Search, but #{ca.name} is a #{ca.type_name}}
          end
          restrict_by_join :id, CardClause.new(ca.get_query).rawclause
        end
      end
