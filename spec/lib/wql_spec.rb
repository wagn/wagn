...
describe Wql do
  ...
  describe "order" do
    ...
    it "should sort by plus card content" do
      User.as :wagbot do
        ...
        w = Wql.new( :right_plus=>'*table of contents', :sort=>{ :right=>'*table_of_contents', :cast=>'integer' } )
  ...
  describe "nested permissions" do
    rule_id = Card.fetch('*all+*read').id
    
    it "are generated by default" do
      Wql.new( { :left=>{:name=>"X"}}).sql.should match(
        "^#{Regexp.escape("(select t.name from cards t  where t.trunk_id in (select tx.id from cards tx  where tx.key = ")}"+
        "E?#{Regexp.escape("'x' and  (tx.read_rule_id IN (#{rule_id}))  and tx.trash is false    ) and  (t.read_rule_id IN (#{rule_id}))  and t.trash is false  ORDER BY t.updated_at desc  )")}$" 
      )
    end
    ...
    it "are not generated inside .without_nested_permissions block" do
       Wql.without_nested_permissions do 
         Wql.new( { :left=>{:name=>"X"}}).sql.should match(
           "^#{Regexp.escape("(select t.name from cards t  where t.trunk_id in (select tx.id from cards tx  where tx.key = ")}"+
           "E?#{Regexp.escape("'x' and tx.trash is false    ) and  (t.read_rule_id IN (#{rule_id}))  and t.trash is false  ORDER BY t.updated_at desc  )")}$" )
      end
    end
