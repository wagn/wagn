...
class RevisionTest < ActiveSupport::TestCase
  ...
  def test_revise
    ...
    User.current_user = Card::WagbotID
    rc1=author_cd1.star_rule(:roles)
    rc1 << Card::AdminID
    rc2 = author_cd2.star_rule(:roles)
    rc2 << Card::AdminID
    author_cd1.save
    author_cd2.save
    User.current_user = author1
    card = newcard( 'alpha', 'stuff')
    User.current_user = author2.card_id
    card.content = 'boogy'
    card.save
    card.reload
    
    assert_equal 2, card.revisions.length, 'Should have two revisions'
    assert_equal author2.card.name, card.current_revision.author.name, 'current author'
    assert_equal author1.card.name, card.revisions.first.author.name,  'first author'
  end
